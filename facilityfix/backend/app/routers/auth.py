...
async def login_user(body: LoginBody) -> Dict[str, Any]:
    ...
    try:
        ...
        # 2) Fetch Firestore profile by UID (document id = Firebase UID)
        prof_ok, profile, _ = await _get_profile_by_uid(user.uid)

        # 🔑 PATCH: fallback to Firestore user_id lookup
        if not prof_ok or not profile:
            profile = await _get_user_doc_by_user_id(user_id)
            if not profile:
                raise HTTPException(status_code=404, detail="User profile not found")

        # 3) Enforce user_id match
        if profile.get("user_id") != user_id:
            raise HTTPException(status_code=403, detail="User ID does not match this email")

        # 4) Enforce role match
        ...
        # 6) Verify password with Firebase
